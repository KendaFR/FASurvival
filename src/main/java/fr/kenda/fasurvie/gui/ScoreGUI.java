...
public class ScoreGUI extends Gui {
    private final Map<UUID, List<ItemStack>> tempStorage = new HashMap<>();
    ...
    public void handleInventoryClick(InventoryClickEvent event) {
        ...
        if (itemToPlace != null && !isFreshCoins(itemToPlace)) {
            event.setCancelled(true);
            event.getWhoClicked().sendMessage(
                    FASurvival.PREFIX + ChatColor.RED + "Seuls les FreshCoins sont autorisés !");
        }
    }

    @Override
    public void handleInventoryDrag(InventoryDragEvent event) {
        boolean dragToCustomInventory = event.getRawSlots().stream()
                .anyMatch(slot -> slot < this.inventory.getSize());

        if (dragToCustomInventory && !isFreshCoins(event.getCursor())) {
            event.setCancelled(true);
            event.getWhoClicked().sendMessage(
                    FASurvival.PREFIX + ChatColor.RED + "Seuls les FreshCoins sont autorisés !");
        }
    ...
    public void handleInventoryClose(InventoryCloseEvent event) {
        ...
        if (!items.isEmpty()) {
            tempStorage.put(player.getUniqueId(), items);
            Bukkit.getScheduler().runTaskAsynchronously(FASurvival.getInstance(),
                    () -> processCoinsAsync(player.getUniqueId()));
        }
    }

    private void processCoinsAsync(UUID playerId) {
        List<ItemStack> items = tempStorage.get(playerId);
        if (items == null) return;

        int totalCoins = items.stream()
                .filter(this::isFreshCoins)
        ...
        Bukkit.getScheduler().runTask(FASurvival.getInstance(), () -> {
            ...
            if (player != null && totalCoins > 0) {
                saveCoinsPlayer(player, totalCoins);
            }
            tempStorage.remove(playerId);
        });
    }
    ...
    private boolean isFreshCoins(ItemStack item) {
        return item != null
                ...
                && item.getItemMeta().getDisplayName().equalsIgnoreCase(Constant.FRESH_COIN);
    }

    private void saveCoinsPlayer(Player player, int coins) {
        player.sendMessage(FASurvival.PREFIX + ChatColor.GOLD +
                "Vous avez déposé " + ChatColor.AQUA + coins + ChatColor.GOLD + " coins.");
