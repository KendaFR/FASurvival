...
public class CraftManager implements IManager {
    ...
    public void register() {
        registerCraft(Constant.FRESH_COIN_ITEM, "fresh_coin");
    }
    ...
    private static void registerCraft(ItemStack freshCoins, String key) {
        ...
        FASurvival.getInstance().getConfig().getConfigurationSection(key +".reference").getKeys(true).forEach(s -> {
            recipe.setIngredient(s.charAt(0), Config.getMaterial(key +".reference." + s));
        });
